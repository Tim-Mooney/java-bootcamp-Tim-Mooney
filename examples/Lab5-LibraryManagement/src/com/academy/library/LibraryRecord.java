...
public class LibraryService {
    ...
    public void borrowBook() {
        ...
        borrowRecords.put(bookId,memId);

        BorrowRecord rec = new BorrowRecord(bookId, memId, LocalDate.now());
        borrowHistory.add(rec);

        if(borrowFrequency.getOrDefault(bookId, 0) == 0){
            borrowFrequency.put(bookId,1);
        }
        else {
            borrowFrequency.put(bookId, borrowFrequency.get(bookId) + 1);
        }
