...
public class CustomerValidator {
    ...
    public void validateNew(Customer customer, String correlationId) {
        if (customer.getCustomerId() == null || customer.getCustomerId().isBlank()) {
            throw BusinessException.conflict("customerId is required", correlationId);
        }
        if (repository.existsById(customer.getCustomerId())) {
            throw BusinessException.conflict("duplicate customerId: " + customer.getCustomerId(), correlationId);
        }
        if (customer.getEmail() != null && repository.existsByEmail(customer.getEmail())) {
            throw BusinessException.conflict("duplicate email: " + customer.getEmail(), correlationId);
        }
    }
